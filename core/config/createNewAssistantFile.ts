...
# To learn more, see the full config.yaml reference: https://docs.ai-firewall.dev/reference
...
# https://docs.ai-firewall.dev/customization/models
models:
  - name: my gpt-5
    provider: openai
    model: gpt-5
...
# MCP Servers that AI Firewall can access
# https://docs.ai-firewall.dev/customization/mcp-tools
mcpServers:
...
export async function createNewAssistantFile(
  ...
  const baseDirUri = joinPathsToUri(
    workspaceDirs[0],
    assistantPath ?? ".ai-firewall/agents",
  );
