...
export function ProvidersTab() {
  ...
  async function fetchData() {
    try {
      const [pRes, mRes] = await Promise.all([
        fetch(`${PROXY_URL}/api/providers`).then((r) => (r.ok ? r.json() : { providers: [] })),
        fetch(`${PROXY_URL}/api/models`).then((r) => (r.ok ? r.json() : { models: [] })),
      ]);
      setProviders(pRes.providers ?? pRes ?? []);
      setModels(mRes.models ?? mRes ?? []);
    } catch {
  ...
  async function handleAdd() {
    ...
    try {
      const res = await fetch(`${PROXY_URL}/api/providers`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        setFormError(typeof d.error === "string" ? d.error : `HTTP ${res.status}`);
        return;
      }
  ...
  async function handleToggle(id: number, enabled: boolean) {
    await fetch(`${PROXY_URL}/api/providers/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ enabled: !enabled }),
    }).catch(() => { setFormError("Operation failed"); });
    await fetchData();
  }

  async function handleDelete(id: number) {
    await fetch(`${PROXY_URL}/api/providers/${id}`, { method: "DELETE" }).catch(() => { setFormError("Operation failed"); });
    await fetchData();
  }
