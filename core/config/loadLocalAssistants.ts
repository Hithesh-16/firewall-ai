import { BLOCK_TYPES } from "@ai-firewall/config-yaml";
...
export function isFirewallAgentConfigFile(uri: string): boolean {
  ...
  return (
    normalizedUri.includes(`/.ai-firewall/agents/`) ||
    normalizedUri.includes(`/.ai-firewall/assistants/`)
  );
}
