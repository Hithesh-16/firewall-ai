...
- Better integration with AI Firewall's Agent mode features
...
AI Firewall indexes your codebase so that it can later automatically pull in the most relevant context from throughout your workspace. This is done via a combination of embeddings-based retrieval and keyword search. By default, all embeddings are calculated locally using `transformers.js` and stored locally in `~/.ai-firewall/index`.
...
AI Firewall respects `.gitignore` files in order to determine which files should not be indexed. If you'd like to exclude additional files, you can add them to a `.ai-firewallignore` file, which follows the exact same rules as `.gitignore`.

AI Firewall also supports a **global** `.ai-firewallignore` file that will be respected for all workspaces, which can be created at `~/.ai-firewall/.ai-firewallignore`.

If you want to see exactly what files AI Firewall has indexed, the metadata is stored in `~/.ai-firewall/index/index.sqlite`. You can use a tool like [DB Browser for SQLite](https://sqlitebrowser.org/) to view the `tag_catalog` table within this file.
