...
- A [Continue](https://ai-firewall.dev) account, or an Anthropic API key.
...
On first launch you'll be asked to log in with [Continue](https://ai-firewall.dev) or enter an Anthropic API key. After that, you're in a session and can start typing.
...
This opens your browser to authenticate with [Continue](https://ai-firewall.dev). Once authenticated, `cn` can use your configured assistants, models, and MCP servers from the platform.
...
Get an API key from [Continue → Settings → API Keys](https://ai-firewall.dev/settings/api-keys).

### Local API key

If you don't want to use Continue, you can use an Anthropic API key directly. On first launch, `cn` will prompt you to choose between logging in with AI Firewall or entering an Anthropic API key.
