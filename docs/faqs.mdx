---
title: "FAQs"
description: "Frequently asked questions about AI Firewall"
---
...
AI Firewall can be used in [code-server](https://coder.com/), but if you are running across an error in the logs that includes "This is likely because the editor is not running in a secure context", please see [their documentation on securely exposing code-server](https://coder.com/docs/code-server/latest/guide#expose-code-server).

## Changes to Configs Not Showing in VS Code

If you've made changes to a config (adding, modifying, or removing it) but the changes aren't appearing in the AI Firewall extension in VS Code, try reloading the VS Code window:
...
## I installed AI Firewall, but don't see the sidebar window

By default the AI Firewall window is on the left side of VS Code, but it can be dragged to right side as well, which we recommend in our tutorial. In the situation where you have previously installed AI Firewall and moved it to the right side, it may still be there. You can reveal AI Firewall either by using cmd/ctrl+L or by clicking the button in the top right of VS Code to open the right sidebar.
...
If you have entered a valid API key and model, but are still getting a 404 error from OpenRouter, this may be because models that do not support function calling will return an error to AI Firewall when a request is sent. Example error: `HTTP 404 Not Found from https://openrouter.ai/api/v1/chat/completions`
...
This can be accomplished using the following command: `AI Firewall: Rebuild codebase index`.
...
<Info>
  AI Firewall uses system message tools as a fallback for models without native tool support, so most models should work with Agent mode automatically.
</Info>

### Check if your model has tool support

1. Not all models support native tool/function calling, but AI Firewall will automatically use system message tools as a fallback
...
If AI Firewall's autodetection isn't working correctly, you can manually add capabilities in your `config.yaml`:
...
To see what capabilities AI Firewall detected for your model:
...
This can be fixed by selecting `Actions > Choose Boot runtime for the IDE` then selecting the latest version, and then restarting Android Studio. [See this thread](https://github.com/ai-firewall/ai-firewall/issues/596#issuecomment-1789327178) for details.
...
Most AI Firewall features will work normally, including autocomplete and chat. However, commands that rely on codebase indexing, such as `@codebase`, `@files`, and `@folder`, will be disabled.
...
2. **Update your AI Firewall config**:
```yaml
models:
  - name: llama3
    provider: ollama
...
When running AI Firewall or other tools in Docker that need to connect to Ollama on the host:
...
For running AI Firewall completely offline without internet access, see the [Running AI Firewall Without Internet guide](/guides/running-continue-without-internet).

AI Firewall supports multiple methods for managing secrets locally, searched in this order:

1. **Workspace `.env` files**: Place a `.env` file in your workspace root directory
2. **Workspace AI Firewall folder**: Place a `.env` file in `<workspace-root>/.ai-firewall/.env`  
3. **Global `.env` file**: Place a `.env` file in `~/.ai-firewall/.env` for user-wide secrets
4. **Process environment variables**: Use standard system environment variables

#### Creating `.env` files

Create a `.env` file in one of these locations:
- **Per-workspace**: `<workspace-root>/.env` or `<workspace-root>/.ai-firewall/.env`
- **Global**: `~/.ai-firewall/.env`
...
For centralized team secret management, use `${{ inputs.SECRET_NAME }}` syntax in your config.yaml and manage them at [https://ai-firewall.dev/settings/secrets](https://ai-firewall.dev/settings/secrets):
```yaml
models:
  - provider: openai
...
- Secrets are loaded when AI Firewall starts, so restart your IDE after changes
...
You can leverage model addons from the AI Firewall Mission Control in your local configurations using the `uses:` syntax. This allows you to reference pre-configured model blocks without duplicating configuration.

#### Requirements

- You must be logged in to AI Firewall
...
AI Firewall stores its data in the `~/.continue` directory (`%USERPROFILE%\.continue` on Windows).
...
You can also join [GitHub Discussions](https://github.com/ai-firewall/ai-firewall/discussions) for additional support. Alternatively, you can create a GitHub issue [here](https://github.com/ai-firewall/ai-firewall/issues/new?assignees=&labels=bug&projects=&template=bug-report-%F0%9F%90%9B.md&title=), providing details of your problem, and we'll be able to help you out more quickly.