...
function DocsIndexingPeek({ status }: DocsIndexingPeekProps) {
  ...
  return (
    <div
      className="text-description-muted flex cursor-pointer flex-row items-center gap-2 rounded-md px-1 hover:bg-list-hover"
      onClick={() => {
        // navigate("/more"); TODO
    ...
    >
      <p className="text-description-muted m-0 p-0 group-hover:underline">
        {status.title}
      </p>
      <div className="my-2 h-1.5 flex-1 rounded-md border border-solid border-border">
        <div
          className={`h-full rounded-lg bg-secondary transition-all duration-200 ease-in-out`}
          style={{
            width: `${progressPercentage}%`,
          }}
        />
      </div>
      <div className="xs:flex text-description-muted hidden flex-row items-center gap-1">
        <span className="text-xs no-underline">
          {progressPercentage.toFixed(0)}%
        </span>
        {/* <ArrowPathIcon
          className={`animate-spin-slow inline-block h-4 w-4 text-description-muted`}
        ></ArrowPathIcon> */}
      </div>
    </div>
  );
