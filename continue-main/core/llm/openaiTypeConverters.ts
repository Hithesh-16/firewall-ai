import { FimCreateParamsStreaming } from "@ai-firewall/openai-adapters/dist/apis/base";
import {
  ChatCompletion,
...
export function toChatMessage(
  ...
  if (message.role === "tool") {
    // tool_call_id is REQUIRED by OpenAI/Groq/Anthropic for role:tool messages.
    // If missing, generate a fallback ID to prevent API errors.
    const toolCallId = message.toolCallId || `call_${Date.now()}`;
    return {
      role: "tool",
