...
import { resolveRequestPolicy } from "../config";
...
// mergeProjectPolicy is now handled inside resolveRequestPolicy
...
export async function registerEstimateRoute(
  app: FastifyInstance,
): Promise<void> {
  app.post(
    "/api/estimate",
    ...
    async (request, reply) => {
      ...
      for (const s of secretResult.secrets) {
        try {
          ...
          if (
            adj &&
            adj.adjustedSeverity &&
            adj.adjustedSeverity !== s.severity
          ) {
      ...
      for (const p of piiResult.pii) {
        try {
          ...
          if (
            adj &&
            adj.adjustedSeverity &&
            adj.adjustedSeverity !== p.severity
          ) {
      ...
      let modelPolicyBlocked:
        | { reason?: string; blockedFiles: string[] }
        | undefined;
      if (modelPolicies) {
      ...
      let promptInjection:
        | {
            score: number;
            isInjection: boolean;
            matches: Array<{ pattern: string; matched: string }>;
          }
        | undefined;
      if (piConfig?.enabled !== false) {
        ...
        promptInjection = {
          ...
          matches: piResult.matches.map((m) => ({
            pattern: m.pattern,
            matched: m.matched,
          })),
        };
      ...
      try {
        if (policy.audit && policy.audit.enabled) {
          ...
          let ghHits = 0;
          if (
            policy.audit.useSurrogateModel &&
            blind.blindMiScore >= (policy.audit.privacyRiskThreshold ?? 0.5)
          ) {
          ...
          const combined = Math.min(
            1,
            blind.blindMiScore + (ghHits > 0 ? 0.3 : 0),
          );
          privacyRisk = {
            blindMiScore: blind.blindMiScore,
            githubHits: ghHits,
            privacyRiskScore: Math.round(combined * 100) / 100,
          };
        }
      } catch (e) {
        // best-effort
      ...
      const estimatedOutputTokens = Math.max(
        256,
        Math.floor(estimatedInputTokens * 0.25),
      );
