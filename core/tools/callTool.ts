...
import { taskBoundaryImpl } from "./implementations/taskBoundary";
      ...
      try {
        ...
        if (fwResp.status === 403) {
          ...
          if (fwData.sanitizedArguments) {
            finalArgs = fwData.sanitizedArguments;
          }
        }
      } catch (e) {
...
export async function callBuiltInTool(
  ...
  switch (functionName) {
    ...
    case BuiltInToolNames.TaskBoundary:
      return await taskBoundaryImpl(args, extras);
    ...
    case BuiltInToolNames.SaveMemory:
      return await saveMemoryImpl(args, extras);
    case BuiltInToolNames.ReadMemory:
      return await readMemoryImpl(args, extras);
    case BuiltInToolNames.CreatePlan:
