...
describe("TipsDisplay", () => {
  ...
  it("renders a random tip from the tips array", () => {
    ...
    const expectedTipIndex = Math.floor(0.5 * AI_FIREWALL_CLI_TIPS.length);
    const expectedTip = AI_FIREWALL_CLI_TIPS[expectedTipIndex];
  ...
  it("renders different tips based on random selection", () => {
    ...
    expect(firstFrame()).toContain(AI_FIREWALL_CLI_TIPS[0]);

    // Test with last tip - use partial match due to line wrapping
    vi.mocked(Math.random).mockReturnValue(0.99);
    const { lastFrame: lastFrame } = render(<TipsDisplay />);
    const lastTip = AI_FIREWALL_CLI_TIPS[AI_FIREWALL_CLI_TIPS.length - 1];
  ...
  it("always renders the tip structure correctly", () => {
    ...
    const containsAtLeastOneTip = AI_FIREWALL_CLI_TIPS.some(
...
describe("AI_FIREWALL_CLI_TIPS", () => {
  it("contains at least 6 tips", () => {
    expect(AI_FIREWALL_CLI_TIPS.length).toBeGreaterThanOrEqual(6);
  });

  it("all tips are non-empty strings", () => {
    AI_FIREWALL_CLI_TIPS.forEach((tip, index) => {
  ...
  it("tips are helpful and informative", () => {
    ...
    const allTipsText = AI_FIREWALL_CLI_TIPS.join(" ").toLowerCase();
