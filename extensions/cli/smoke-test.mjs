...
runTest("Local packages are bundled", () => {
  ...
  // Check for code from @ai-firewall/config-yaml
  if (
    !bundleContent.includes("AssistantUnrolled") &&
    !bundleContent.includes("config-yaml")
  ) {
    throw new Error("@ai-firewall/config-yaml not properly bundled");
  }

  // Check for code from @ai-firewall/openai-adapters
  ...
  if (
    ...
    throw new Error("@ai-firewall/openai-adapters not properly bundled");
  }
});
