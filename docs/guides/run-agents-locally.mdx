...
description: "Set up version-controlled agents in your repository and run them with AI Firewall CLI or GitHub Actions"
---
...
<Card title="What You'll Build" icon="folder-open">
  ...
  - Runs agents with AI Firewall CLI for local development
...
- [AI Firewall CLI installed](/cli/quickstart)
...
<Steps>
  ...
  <Step title="Create your first agent">
    ...
    ```bash
    touch .ai-firewall/agents/my-agent.md
    ```
  ...
  <Step title="Run the agent">
    ```bash
    cn -p --agent .ai-firewall/agents/my-agent.md
    ```

    For interactive development (TUI mode):

    ```bash
    cn --agent .ai-firewall/agents/my-agent.md
    ```
  </Step>
</Steps>
...
AI Firewall provides a [reusable workflow](https://github.com/ai-firewall/ai-firewall/blob/main/.github/workflows/continue-agents.yml) that handles agent discovery, parallel execution, and GitHub Check reporting.
...
jobs:
  agents:
    uses: ai-firewall/ai-firewall/.github/workflows/continue-agents.yml@main
    secrets:
      ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
```

The reusable workflow automatically:
- Discovers all `.md` files in `.ai-firewall/agents/`
...
jobs:
  agents:
    uses: ai-firewall/ai-firewall/.github/workflows/continue-agents.yml@main
    with:
      agents-path: '.ai-firewall/agents'  # Custom agents directory (optional)
    secrets:
      ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
```
...
<Tabs>
  ...
  <Tab title="CLI Tool Permissions">
    ...
    cn --allow "Read()" --allow "Grep()" --exclude "Bash()" \
       --agent .ai-firewall/agents/analysis-only.md
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Agent runs but doesn't complete the task">
    ...
    - Enable verbose logging: `cn --verbose --agent .ai-firewall/agents/my-agent.md`
    - Review logs with `cn --verbose` or check the AI Firewall logs directory
  </Accordion>
  ...
  <Accordion title="Workflow doesn't discover agents">
    ...
    ```bash
    find .ai-firewall/agents -name "*.md" -type f
    ```

    The workflow looks for `.md` files in `.ai-firewall/agents/`.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="CLI Reference" icon="terminal" href="/cli/quickstart">
    Full AI Firewall CLI documentation
  </Card>
