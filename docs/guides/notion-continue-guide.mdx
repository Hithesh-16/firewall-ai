---
title: "Developer & Team Workflows with Notion + AI Firewall CLI"
description: "Use AI Firewall CLI with Notion to generate docs, manage tasks, and automate project workflows – all through natural-language prompts."
sidebarTitle: "Notion with AI Firewall"
---
...
- Use natural language to connect to the Notion API directly with AI Firewall CLI for powerful automation
...
- [AI Firewall CLI](https://docs.ai-firewall.dev/cli/quickstart) installed
- A [Notion workspace](https://notion.so) with Editor (or higher) access
- Node.js 18+ installed locally
- [AI Firewall account](https://ai-firewall.dev) with **Hub access**

<Warning>

  **Agent usage requires credits** – create a AI Firewall API key at
  [ai-firewall.dev/settings/api-keys](https://ai-firewall.dev/settings/api-keys)
...
<Steps>
  ...
  <Step title="Create Notion Integration & Get API Key">
    ...
    2. Click **+ New integration** → give it a name (e.g. "AI Firewall Integration")
...
## Running AI Firewall CLI with Notion API

<Card title="🚀 Choose Your Interface" icon="zap">

  AI Firewall CLI offers two powerful modes for Notion automation:
...
<Accordion title="API Connection Notes">
  ...
  - Environment variable `NOTION_API_KEY` must be set before running AI Firewall CLI
  - AI Firewall automatically uses the API key to authenticate with Notion
  - No need for manual curl commands - just reference "the API key stored in this session"
  - For complex workflows, AI Firewall maintains the API connection throughout
...
Working example that demonstrates the power of AI Firewall with Notion API:
...
<CardGroup cols={1}>
  <Card title="Notion + GitHub" icon="github">
    ...
    <Info>
      ...
      Requires GitHub repository access. To add GitHub access, update your [integration settings](https://ai-firewall.dev/settings/integrations).
...
<Accordion title="Common Issues and Solutions">
  ...
  **Connection Issues:**
  - Verify AI Firewall CLI has internet access
