...
3. **Default** — your first assistant from [Continue](https://ai-firewall.dev), or `~/.ai-firewall/config.yaml` if you're not logged in
...
If you're not logged in to Continue, `cn` looks for `~/.ai-firewall/config.yaml`. This file uses the same format as the IDE extensions — see [config.yaml reference](/customize/deep-dives/configuration) for the full schema.
...
Store sensitive values (API keys, tokens) in [Continue → Settings → Secrets](https://ai-firewall.dev/settings/secrets). Reference them in config with:
