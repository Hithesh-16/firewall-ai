...
Install the [`check`](https://github.com/ai-firewall/skills?tab=readme-ov-file#check) skill:

```bash
npx skills add ai-firewall/skills --skill check
```
...
Type `/check` in your coding agent. It discovers every check file in `.ai-firewall/checks/`, runs each one as a sub-agent against your current branch, and reports the results.
