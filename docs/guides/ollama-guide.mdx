---
title: "Using Ollama with AI Firewall: A Developer's Guide"
description: "Complete guide to setting up Ollama with AI Firewall for local AI development. Learn installation, configuration, model selection, performance optimization, and troubleshooting for privacy-focused offline coding assistance"
---
...
- AI Firewall extension installed
...
## How to Configure Ollama with AI Firewall

There are multiple ways to configure Ollama models in AI Firewall:

### Method 1: Using Hub Model Blocks in Local config.yaml

The easiest way is to use [pre-configured model blocks](/reference#models) from the AI Firewall Mission Control in your local configuration:

```yaml title="~/.ai-firewall/configs/config.yaml"
...
<Warning>
  **Important**: Blocks only provide configuration - you still need to pull
  the model locally. The block `ollama/deepseek-r1-32b` configures AI Firewall
  to use `model: deepseek-r1:32b`, but the actual model must be installed:
  ```bash
  # Check what the block expects (view on ai-firewall.dev)
...
AI Firewall can automatically detect available Ollama models. You can configure this in your YAML:
...
3. AI Firewall will scan for available Ollama models
...
<Note>
  ...
  available models. When set to `AUTODETECT`, AI Firewall will dynamically populate
...
```yaml
# ~/.ai-firewall/configs/config.yaml
name: Local Config
version: 0.0.1
schema: v1
models:
...
async def create_user(user: User):
    # AI Firewall will help complete this implementation
...
Use AI Firewall with Ollama to:
...
Ollama with AI Firewall provides a powerful local development environment for AI-assisted coding. You now have complete control over your AI models, ensuring privacy and enabling offline development workflows.

---

_This guide is based on Ollama v0.11.x and AI Firewall v1.1.x. Please check for updates regularly._
