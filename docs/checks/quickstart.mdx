...
```
Help me write checks for this codebase: https://ai-firewall.dev/walkthrough
```

The walkthrough reads your codebase, helps you write your first checks, connects you to AI Firewall, and gets them running on your next PR. It takes a few minutes.

If you'd rather set things up manually, keep reading.

## Writing checks manually

A check file has two parts: **frontmatter** (YAML between `---` delimiters) with a required `name` and `description`, and a **body** prompt that tells the AI what to look for. Create a `.md` file in `.ai-firewall/checks/`:

```markdown .ai-firewall/checks/security-review.md
---
