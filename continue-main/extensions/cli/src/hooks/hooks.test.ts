...
describe("hookConfig", () => {
  ...
  describe("loadHooksConfig", () => {
    ...
    beforeEach(() => {
      ...
      // Override AI_FIREWALL_GLOBAL_DIR so that user-global settings
      // from the real ~/.ai-firewall/settings.json don't leak into tests
      originalContinueGlobalDir = process.env.AI_FIREWALL_GLOBAL_DIR;
      process.env.AI_FIREWALL_GLOBAL_DIR = path.join(fakeHome, ".ai-firewall");
    });
    ...
    afterEach(() => {
      if (originalContinueGlobalDir === undefined) {
        ...
        process.env.AI_FIREWALL_GLOBAL_DIR = originalContinueGlobalDir;
      }
