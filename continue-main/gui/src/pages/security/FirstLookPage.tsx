...
function FirstLookPage() {
  ...
  async function fetchDetectedPatterns() {
    try {
      const res = await fetch(`${PROXY_URL}/api/perimeter/detect`);
      if (res.ok) {
        const data = await res.json();
        const detected = data.patterns as DetectedPattern[];
        setPatterns(detected);
        // Pre-select recommended + already active patterns
        ...
        setError("Could not reach the AI Firewall proxy.");
      }
    } catch {
      setError("Proxy is not running. Start it on port 8080.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function confirmPerimeter() {
    setConfirming(true);
    try {
      const res = await fetch(`${PROXY_URL}/api/perimeter/confirm`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ restricted: Array.from(selected) }),
      });
      if (res.ok) {
        navigate(ROUTES.SECURITY);
      } else {
        setError("Failed to save security perimeter.");
      }
    } catch {
