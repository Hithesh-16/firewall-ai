---
title: "How to Configure AI Firewall"
description: Learn how to access and manage AI Firewall configurations through Hub or local YAML files
keywords: [config, settings, customize]
sidebarTitle: "Configuration"
---

You can easily access your configuration from the AI Firewall Chat sidebar. Open the sidebar by pressing <kbd>cmd/ctrl</kbd> + <kbd>L</kbd> (VS Code) or <kbd>cmd/ctrl</kbd> + <kbd>J</kbd> (JetBrains) and click the Agent selector above the main chat input. Then, you can hover over an agent and click the `new window` (hub agents) or `gear` (local agents) icon.

![configure](/images/configure-continue.png)

## How to Manage Hub Configs

  Hub Configs can be managed in [the Hub](https://ai-firewall.dev). See [Editing a config](/mission-control/configs/edit-a-config)
...
- `~/.ai-firewall/config.yaml` (MacOS / Linux)
- `%USERPROFILE%\.ai-firewall\config.yaml` (Windows)
...
When editing this file, you can see the available options suggested as you type, or check the reference below. When you save a config file from the IDE, AI Firewall will automatically refresh to take into account your changes. A config file is automatically created the first time you use AI Firewall, and always automatically generated with default values if it doesn't exist.
...
- `config.ts` - Advanced configuration (probably unnecessary) - a TypeScript file in your home directory that can be used to programmatically modify (_merged_) the `config.json` schema:
  - `~/.ai-firewall/config.ts` (MacOS / Linux)
  - `%USERPROFILE%\.ai-firewall\config.ts` (Windows)
...
`config.yaml` or `config.json` can handle the vast majority of necessary configuration, so we recommend using it whenever possible. However, if you need to programmatically extend AI Firewall configuration, you can use a `config.ts` file, placed at `~/.ai-firewall/config.ts` (MacOS / Linux) or `%USERPROFILE%\.ai-firewall\config.ts` (Windows).
