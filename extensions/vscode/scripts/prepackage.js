...
void (async () => {
  ...
  // Try root node_modules first (npm hoists), fall back to core/node_modules
  ...
  // Try root node_modules first (npm hoists), fall back to core/node_modules
  const treeSitterCandidates = [
    path.join(__dirname, "../../../node_modules/tree-sitter-wasms/out"),
    path.join(__dirname, "../../../core/node_modules/tree-sitter-wasms/out"),
  ];
  const treeSitterSrc = treeSitterCandidates.find((p) => fs.existsSync(p));
  if (!treeSitterSrc) {
    console.warn("[info] tree-sitter-wasms/out not found, skipping");
  } else {
    await new Promise((resolve, reject) => {
      ncp(
        treeSitterSrc,
