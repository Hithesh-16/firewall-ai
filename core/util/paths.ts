...
import { ConfigYaml, DevEventName } from "@ai-firewall/config-yaml";
...
const AI_FIREWALL_GLOBAL_DIR = (() => {
  const configPath = process.env.AI_FIREWALL_GLOBAL_DIR;
  if (configPath) {
  ...
  return path.join(os.homedir(), ".ai-firewall");
})();
...
export function getAiFirewallGlobalPath(): string {
  // This is ~/.ai-firewall on mac/linux
  const continuePath = AI_FIREWALL_GLOBAL_DIR;
...
export function getIndexFolderPath(): string {
  const indexPath = path.join(getAiFirewallGlobalPath(), "index");
...
export function getSharedConfigFilePath(): string {
  return path.join(getAiFirewallGlobalPath(), "sharedConfig.json");
}
...
export function getConfigJsonPath(): string {
  const p = path.join(getAiFirewallGlobalPath(), "config.json");
  return p;
}
...
export function getConfigTsPath(): string {
  const p = path.join(getAiFirewallGlobalPath(), "config.ts");
  if (!fs.existsSync(p)) {
    fs.writeFileSync(p, DEFAULT_CONFIG_TS_CONTENTS);
  }

  const typesPath = path.join(getAiFirewallGlobalPath(), "types");
  ...
  const packageJsonPath = path.join(getAiFirewallGlobalPath(), "package.json");
  if (!fs.existsSync(packageJsonPath)) {
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        description: "My AI Firewall Configuration",
        main: "config.js",
      }),
    );
  }
...
export function getConfigJsPath(): string {
  ...
  return path.join(getAiFirewallGlobalPath(), "out", "config.js");
}

export function getTsConfigPath(): string {
  const tsConfigPath = path.join(getAiFirewallGlobalPath(), "tsconfig.json");
...
function getDevDataPath(): string {
  const sPath = path.join(getAiFirewallGlobalPath(), "dev_data");
...
function getMigrationsFolderPath(): string {
  const migrationsPath = path.join(getAiFirewallGlobalPath(), ".migrations");
...
export function getRemoteConfigsFolderPath(): string {
  const dir = path.join(getAiFirewallGlobalPath(), ".configs");
...
export function getAiFirewallDotEnv(): { [key: string]: string } {
  const filepath = path.join(getAiFirewallGlobalPath(), ".env");
  if (fs.existsSync(filepath)) {
...
export function getLogsDirPath(): string {
  const logsPath = path.join(getAiFirewallGlobalPath(), "logs");
...
export function getGlobalFolderWithName(name: string): string {
  return path.join(getAiFirewallGlobalPath(), name);
}
...
export function getLocalEnvironmentDotFilePath(): string {
  return path.join(getAiFirewallGlobalPath(), ".local");
}

export function getStagingEnvironmentDotFilePath(): string {
  return path.join(getAiFirewallGlobalPath(), ".staging");
}

export function getDiffsDirectoryPath(): string {
  const diffsPath = path.join(getAiFirewallGlobalPath(), ".diffs"); // .replace(/^C:/, "c:"); ??
