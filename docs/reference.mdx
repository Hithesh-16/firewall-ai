...
description: "Comprehensive guide to the config.yaml format used by AI Firewall.dev for building custom coding agents. Learn how to define models, context providers, rules, prompts, and more using YAML configuration."
---

## Introduction

AI Firewall Agents are defined using the `config.yaml` specification.
...
Learn how to work with AI Firewall's configuration system, including using hub models, rules, and tools, creating local configurations, and organizing your setup.
</Card>
...
- `capabilities`: Array of strings denoting model capabilities, which will overwrite AI Firewall's autodetection based on provider and model. See the [Model Capabilities guide](/customize/deep-dives/model-capabilities) for detailed information. Supported capabilities include:
  ...
  AI Firewall automatically detects these capabilities for most models, but you can override this when using custom deployments or if autodetection isn't working correctly.
...
schema: v1
rules:
  - uses: sanity/sanity-opinionated # rules file stored on AI Firewall Mission Control
...
schema: v1
prompts:
  - uses: supabase/create-functions # prompts file stored on AI Firewall Mission Control 
...
docs:
  - name: AI Firewall
    startUrl: https://docs.ai-firewall.dev/intro
    favicon: https://docs.ai-firewall.dev/favicon.ico
```
...
The [Model Context Protocol](https://modelcontextprotocol.io/introduction) is a standard proposed by Anthropic to unify prompts, context, and tool use. AI Firewall supports any MCP server with the MCP context provider.
...
data:
  - name: Local Data Bank
    destination: file:///Users/dallin/Documents/code/ai-firewall/ai-firewall-extras/external-data
    schema: 0.2.0
    level: all
