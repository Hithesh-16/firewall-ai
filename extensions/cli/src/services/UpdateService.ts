...
export class UpdateService extends BaseService<UpdateServiceState> {
  ...
  private async checkAndAutoUpdate() {
    ...
    try {
      ...
      if (
        ...
        !process.env.AI_FIREWALL_CLI_AUTO_UPDATED //Already auto updated, preventing sequential auto-update
      ) {
        ...
        this.setState({
          ...
          message: isUpdateAvailable
            ...
            : `AI Firewall CLI v${this.currentState.currentVersion}`,
          isUpdateAvailable,
          latestVersion,
        });
      }
      ...
      this.setState({
        ...
        message: `AI Firewall CLI v${this.currentState.currentVersion}`,
        error,
      });
    }
  }
  ...
  async performUpdate(isAutoUpdate?: boolean) {
    ...
    try {
      ...
      // Install the update
      const { stdout, stderr } = await execAsync("npm i -g @ai-firewall/cli");
  ...
  private restartCLI(): void {
    try {
      ...
      const child = spawn(nodeExecutable, [entryPoint, ...cliArgs], {
        ...
        env: {
          ...process.env,
          AI_FIREWALL_CLI_AUTO_UPDATED: "true",
        },
      });
