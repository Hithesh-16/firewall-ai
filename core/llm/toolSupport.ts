import { parseProxyModelName } from "@ai-firewall/config-yaml";
...
export const PROVIDER_TOOL_SUPPORT: Record<string, (model: string) => boolean> =
  {
    ...
    groq: (model) => {
      const lower = model.toLowerCase();
      if (lower.includes("tool-use") || lower.includes("tool_use")) return true;
      if (/llama.*(3\.[1-9]|[4-9]\.)/.test(lower)) return true;
      if (lower.includes("mixtral") || lower.includes("gemma")) return true;
      if (lower.includes("70b") || lower.includes("90b")) return true;
      return false;
    },
  };
