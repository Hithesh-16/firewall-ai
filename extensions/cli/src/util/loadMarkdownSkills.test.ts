...
vi.mock("../env.js", () => ({
  env: {
    continueHome: "/mock/home/.ai-firewall",
  },
}));
...
`,
  ...
  it("returns error for invalid frontmatter", async () => {
    const skillDir = path.join(tmpDir, ".ai-firewall", "skills", "bad-skill");
    fs.mkdirSync(skillDir, { recursive: true });
    fs.writeFileSync(
      path.join(skillDir, "SKILL.md"),
      `---
...
`,
  ...
  it("loads multiple skills from different directories", async () => {
    const skill1Dir = path.join(tmpDir, ".ai-firewall", "skills", "skill-1");
    const skill2Dir = path.join(tmpDir, ".ai-firewall", "skills", "skill-2");
