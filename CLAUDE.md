...
- **Central Scanning Chokepoint** — `core/util/scanning/ScanningIde.ts` wraps the `IDE` interface at both construction sites (`extensions/vscode/src/extension/VsCodeExtension.ts` and `binary/src/index.ts`) so every `ide.readFile`/`readRangeInFile`/`getCurrentFile` call across the monorepo — context providers, tools, indexing, autocomplete, retrieval — flows through one decorator. Replaces the old per-site scan calls in `CurrentFileContextProvider`/`FileContextProvider`/`OpenFilesContextProvider`/`readFile`/`readFileRange`/`readCurrentlyOpenFile`. Callers tag purpose via `readFileWith(ide, uri, purpose)` or let it default to the safe `"llm"`. `ScanPurpose = "llm" | "indexing" | "autocomplete" | "config" | "raw"` drives the decision matrix: `llm` enforces (BLOCK throws `FileBlockedByScanError`, REDACT substitutes, report emits); `indexing` uses a short 1s proxy timeout and degrades BLOCK to empty string so one bad file can't jam a batch; `autocomplete` is cache-only (never calls the proxy synchronously — 50-file snippet fetches stay <100ms, asserted by a regression test); `config` bypasses entirely for the firewall's own rule/ignore files; `raw` is an explicit escape hatch. A hardcoded allowlist (`.gitignore`, `.ai-firewallignore`, `plugin.json`, `policy.json`, `.ai-firewall/config.yaml`, etc.) forces `"config"` even if the caller tags `"llm"` — defense-in-depth against self-referential blocks. CLI tools get a parallel `extensions/cli/src/services/ScanningFileIo.ts` shim with the same decision matrix, same `ScanPurpose`. The LRU decision cache (`scanDecisionCache.ts`, capacity 5000, key = `path:mtime:purpose`) caches decisions in-process so repeated reads never round-trip the proxy twice.
- **Scan Report Channel** — `core/util/scanning/scanReportChannel.ts` is a per-session pub/sub that threads scan reports from wherever in the stack a file was read up to the chat layer. Scoped by `AsyncLocalStorage<{correlationId}>`: `core/core.ts` wraps `getContextItems` handlers in `runInScanContext(correlationId, fn)`, subscribes for the turn, and on return merges dedup-by-path reports as extra `ContextItem`s via `reportToContextItem.ts`. Crosstalk between concurrent turns is impossible — subscribers only see their own correlation id. Reports published outside any context are silently dropped, so background reads (startup, indexing) never leak.
- **Scan Result Display** — CLI `ScanBanner` and GUI `ScanResultBanner` show scan findings with masked malicious content in red. Findings sent via `X-AF-Findings` header (compact JSON: `{t,s,c,v,f?,l?,col?}` — type, severity, category, masked value, optional file/line/column). Masking: keys show first 4 + last 2 chars, emails mask local part, phone/SSN show last 4.
- **Inline File Scan Reports** — File-read paths (context providers, `readFile`/`readFileRange`/`readCurrentlyOpenFile` tools, CLI counterparts) route through the central `ScanningIde` chokepoint. When a scan produces findings, the decorator publishes a `ScanReport` to the scan report channel; `core.ts` drains the channel at turn end and attaches an "AI Firewall" `ContextItem` per affected file, powered by `core/util/formatScanFindings.ts`. BLOCK throws `FileBlockedByScanError` carrying the full report — tools translate it into a user-visible `ContinueError` with the markdown breakdown. One source of truth for all scan reporting, no per-site duplication.
...
| `fileScanProxy.ts`           | HTTP client for `/api/scan/file` with `ScanPurpose` + cache-only support           | `core/util/`                        |
| `ScanningIde.ts`             | Central decorator wrapping `IDE` file methods — one scan chokepoint for all reads  | `core/util/scanning/`               |
| `ScanPurpose.ts`             | 5-value purpose enum + decision predicates (llm/indexing/autocomplete/config/raw)  | `core/util/scanning/`               |
| `scanReportChannel.ts`       | `AsyncLocalStorage`-scoped pub/sub threading scan reports to the chat layer        | `core/util/scanning/`               |
| `scanDecisionCache.ts`       | In-process LRU (cap 5000) keyed by path+mtime+purpose, hot-entry survival          | `core/util/scanning/`               |
| `FileBlockedByScanError.ts`  | Typed error for BLOCK with attached `ScanReport` (consent-flow hook point)         | `core/util/scanning/`               |
| `sliceByLines.ts`            | Whole-line slicing helper for `readRangeInFile` on redacted content                | `core/util/scanning/`               |
| `reportToContextItem.ts`     | Convert a `ScanReport` into the "AI Firewall" chat context item                    | `core/util/scanning/`               |
| `ScanningFileIo.ts`          | CLI parallel to `ScanningIde` — same decision matrix for CLI `fs.readFileSync`     | `extensions/cli/src/services/`      |
...
- File scanning is centralised in `core/util/scanning/ScanningIde.ts` — every `ide.readFile`/`readRangeInFile`/`getCurrentFile` call automatically routes through it. Tag hot paths via `readFileWith(ide, uri, purpose)` with `"indexing"`, `"autocomplete"`, or `"config"` to opt out of `llm`-grade enforcement. `"raw"` is an explicit escape hatch that MUST include a `// scan-raw:` comment explaining why byte-exact content is required.
- CLI file I/O goes through `extensions/cli/src/services/ScanningFileIo.ts` for the same decision matrix — never call `fs.readFileSync` directly on project files from a CLI tool that feeds an LLM.
- Scan reports reach the chat via the `scanReportChannel` + `AsyncLocalStorage` correlation id — never add a second per-site "AI Firewall" `ContextItem` push, it will dedupe but is wasted work.
...
- Call `scanFileViaProxy` directly from a context provider, tool, or autocomplete helper — the `ScanningIde` decorator is the only entry point; calling the HTTP client directly is a per-site scan site we've already deleted once.
- Write per-site code that builds its own "AI Firewall" `ContextItem` — the scan report channel delivers reports to the chat layer automatically. Duplicating this logic is what the Step 6 refactor removed.
- Call `ide.readFile` on `.gitignore`, `.ai-firewallignore`, `plugin.json`, `policy.json`, or any `.ai-firewall/config.yaml` without the `"config"` purpose — the decorator's hardcoded allowlist will override the tag, but explicit is better than implicit.
- Make autocomplete helpers synchronously wait for the proxy — the `"autocomplete"` purpose is cache-only and must stay that way. A regression test (`util/scanning/autocompleteLatency.vitest.ts`) asserts 50 reads take <100ms with a 50ms-per-call mocked proxy.
