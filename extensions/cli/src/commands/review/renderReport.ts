...
function renderTextReport(
  ...
  if (options.checksFromHub) {
    lines.push(
      isTTY
        ? chalk.dim("These reviews also run on your PRs via AI Firewall CI.")
        : "These reviews also run on your PRs via AI Firewall CI.",
    );
  } else {
    lines.push(
      isTTY
        ? chalk.dim(
            "Tip: Run these automatically on every PR → https://ai-firewall.dev",
          )
        : "Tip: Run these automatically on every PR → https://ai-firewall.dev",
    );
  }
