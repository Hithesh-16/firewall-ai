...
3. **Default** — your first assistant from [AI Firewall](https://ai-firewall.dev), or `~/.ai-firewall/config.yaml` if you're not logged in
...
If you're not logged in to AI Firewall, `cn` looks for `~/.ai-firewall/config.yaml`. This file uses the same format as the IDE extensions — see [config.yaml reference](/customize/deep-dives/configuration) for the full schema.
...
If you belong to multiple organizations on AI Firewall, use `--org` to specify which one:
...
Store sensitive values (API keys, tokens) in [AI Firewall → Settings → Secrets](https://ai-firewall.dev/settings/secrets). Reference them in config with:
