...
export function useProxyForUnrenderedSecrets(
  ...
  if (config.models) {
    for (let i = 0; i < config.models.length; i++) {
      // AI Firewall vault reference: apiKeyRef = "vault://provider/model"
      // Route through proxy which resolves the vault key at request time
      const vaultRef = (config.models[i] as Record<string, unknown>)?.apiKeyRef as string | undefined;
      if (vaultRef && vaultRef.startsWith("vault://")) {
        config.models[i] = {
          ...config.models[i],
          ...
          apiKeyLocation: vaultRef,
          orgScopeId,
          onPremProxyUrl: onPremProxyUrl ?? "http://localhost:8080",
          apiKey: undefined,
        };
        continue;
      }
