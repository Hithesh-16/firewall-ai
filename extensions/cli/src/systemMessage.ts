...
const baseSystemMessage = `You are an agent in the AI Firewall CLI. Given the user's prompt, you should use the tools available to you to answer the user's question.
...
export async function constructSystemMessage(
  ...
  if (mode === "plan") {
    ...
    if (!process.env.AI_FIREWALL_CLI_DISABLE_COMMIT_SIGNATURE) {
      systemMessage += `\n<context name="commitSignature">When creating commits using any CLI or tool, include the following in the commit message:
Generated with [AI Firewall](https://ai-firewall.dev)

Co-Authored-By: AI Firewall <noreply@ai-firewall.dev>
</context>\n`;
    }
  }
