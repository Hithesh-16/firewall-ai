...
// ── Idempotency Locking ───────────────────────────────────────────────────────
// Multiple tool calls finishing simultaneously can each trigger a new Turn 2
// in parallel. We track active session turns at the module level to ensure only
// one instance of streamNormalInput proceeds for a given sessionId at a time.
// Since JS is single-threaded, checking/adding to this Set is atomic before
// any async `await` allows a race.
...
>(
  ...
  async (
    ...
    try {
      ...
      // Clear any stale firewall activity from previous requests
      dispatch(clearFirewallActivity());

      // Firewall activity lifecycle — show scanning stages in chat
      const emitActivity = (step: FirewallActivityStep, detail?: string) => {
        dispatch(
          setFirewallActivity({
            step,
            message: step,
      ...
      // Show scanning stages sequentially — each visible ~250ms before next
      const delay = (ms: number) => new Promise((r) => setTimeout(r, ms));

      emitActivity("scanning_secrets");
      await delay(250);
      emitActivity("scanning_pii");
      await delay(200);
      emitActivity("scanning_injection");
      await delay(200);
      emitActivity("checking_policy");
      await delay(150);
      ...
      // Clear firewall activity on completion
      emitActivity("complete");
      setTimeout(() => dispatch(clearFirewallActivity()), 2000);
      ...
      if (originalToolCalls.length === 0) {
        ...
        if (builtInReadonlyAutoApproved.length > 0) {
          ...
          // We do NOT await here to avoid deadlocking with the next turn
          // triggered by these calls. Parent turn must finish to release lock.
          builtInReadonlyAutoApproved.forEach(({ toolCallState }) => {
            dispatch(
              callToolById({
        ...
        if (generatedCalls4.length > 0) {
          // We do NOT await here to avoid deadlocking with the next turn
          // triggered by these calls. Parent turn must finish to release lock.
          generatedCalls4.forEach(({ toolCallId }) => {
            dispatch(
              callToolById({
                toolCallId,
                isAutoApproved: true,
