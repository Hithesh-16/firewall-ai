...
AI Firewall CLI (`cn`) is a terminal-based coding agent. It can edit files, run commands, and work through multi-step tasks — the same agent that powers the AI Firewall IDE extensions, running in your terminal.
...
- An [AI Firewall](https://ai-firewall.dev) account, or an Anthropic API key.
...
On first launch you'll be asked to log in with [AI Firewall](https://ai-firewall.dev) or enter an Anthropic API key. After that, you're in a session and can start typing.

## Authentication

### Log in with AI Firewall

```bash
cn login
```

This opens your browser to authenticate with [AI Firewall](https://ai-firewall.dev). Once authenticated, `cn` can use your configured assistants, models, and MCP servers from the platform.
...
Get an API key from [AI Firewall → Settings → API Keys](https://ai-firewall.dev/settings/api-keys).

### Local API key

If you don't want to use AI Firewall, you can use an Anthropic API key directly. On first launch, `cn` will prompt you to choose between logging in with AI Firewall or entering an Anthropic API key.
