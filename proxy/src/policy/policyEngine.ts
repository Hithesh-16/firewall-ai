...
export function evaluatePolicy(
  ...
  if (policy.approval?.enabled && riskScore >= (policy.approval.riskThreshold ?? 50)) {
    return {
      action: "REQUIRE_APPROVAL",
      reasons: [`Risk score ${riskScore} exceeds approval threshold ${policy.approval.riskThreshold ?? 50}`],
      riskScore,
      filesBlocked
    };
  }
