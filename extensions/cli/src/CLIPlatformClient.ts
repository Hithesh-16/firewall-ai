...
import { DefaultApiInterface } from "@ai-firewall/sdk/dist/api";
...
export class CLIPlatformClient implements PlatformClient {
  ...
  private findSecretInLocalEnvFiles(fqsn: FQSN): SecretResult | undefined {
    ...
    // Then check in priority order: ~/.ai-firewall/.env, <workspace>/.ai-firewall/.env, <workspace>/.env
    const workspaceDir = process.cwd();
    const envPaths = [
      path.join(env.continueHome, ".env"),
      path.join(workspaceDir, ".ai-firewall", ".env"),
