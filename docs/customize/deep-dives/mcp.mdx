---
title: "How to Set Up Model Context Protocol (MCP) in AI Firewall"
...
[here](https://ai-firewall.dev/explore/mcp).
...
```yaml title=".ai-firewall/mcpServers/playwright-mcp.yaml"
...
## How to Set Up AI Firewall Documentation Search with MCP

You can set up an MCP server to search the AI Firewall documentation directly from your config. This is particularly useful for getting help with AI Firewall configuration and features.

For complete setup instructions, troubleshooting, and usage examples, see the [AI Firewall MCP Reference](/reference/ai-firewall-mcp).

## Using JSON MCP Format from Claude, Cursor, Cline, etc

<Info>
If you're coming from another tool that uses JSON MCP format configuration files (like Claude Desktop, Cursor, or Cline), you can copy those JSON config files directly into your `.ai-firewall/mcpServers/` directory (note the plural "Servers") and AI Firewall will automatically pick them up.

For example, place your JSON MCP config file at `.ai-firewall/mcpServers/mcp.json` in your workspace.
</Info>
...
`mcpServers`](https://ai-firewall.dev/new?type=block&blockType=mcpServers) or add a local MCP
...
<Note>
When creating a standalone block file in `.ai-firewall/mcpServers/`, remember to include the required metadata fields (`name`, `version`, `schema`) as shown in the Quick Start example above.
</Note>
...
as well as access hosted secrets in the AI Firewall Mission Control. To leverage Hub secrets, you can use the `inputs` property in your MCP env block instead of `secrets`.
