...
export async function registerStatsRoute(app: FastifyInstance): Promise<void> {
  app.get("/api/stats", async () => {
    ...
    const totals = totalsRow ?? { total: 0, blocked: 0, redacted: 0, allowed: 0, avgRiskScore: 0, totalEntropy: 0 };

    const byDayRow = db.select({
      date: sql<string>`DATE(${logs.timestamp} / 1000, 'unixepoch')`,
      count: sql<number>`COUNT(*)`
    }).from(logs).groupBy(sql`date`).orderBy(sql`date DESC`).limit(30).all();

    const byDay = byDayRow as Array<{ date: string; count: number }>;

    const reasonRows = db.select({ reasons: logs.reasons }).from(logs)
  ...
  app.get("/api/risk-score", async () => {
    ...
    }).from(logs).where(gt(logs.timestamp, oneWeekAgo)).get();
