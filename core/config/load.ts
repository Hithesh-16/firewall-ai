...
async function handleEsbuildInstallation(
  ...
  const installCmd = "npm i esbuild@x.x.x --prefix ~/.ai-firewall";
  ...
  try {
    ...
    // Try resolving from ~/.ai-firewall/node_modules as a courtesy
    try {
      ...
      require(candidate);
      return true; // available via ~/.ai-firewall
    } catch {
      // Not available → show friendly instructions and opt out of building
      await ide.showToast(
        "error",
        [
          "config.ts has been deprecated and esbuild is no longer automatically installed by AI Firewall.",
...
async function tryBuildConfigTs() {
  try {
    ...
    console.log(
      `Build error. Please check your ~/.ai-firewall/config.ts file: ${e}`,
    );
  }
}
