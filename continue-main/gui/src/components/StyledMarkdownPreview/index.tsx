  ...
  const handleMouseUp = () => {
    ...
    if (selection && !selection.isCollapsed) {
      ...
      if (text.length > 0) {
        ...
        // Position tooltip centrally above the selection
        setSelectionBox({
          top: rect.top - 30, // 30px above
    ...
    <div onMouseUp={handleMouseUp} style={{ position: "relative" }}>
      {selectionBox && (
        <div
          ...
          onClick={(e) => {
            e.stopPropagation();
            // Send selected text to VSCode, which will handle the quoting logic or send back to input
            ideMessenger.post("insertAtCursor", { text: `> ${selectionBox.text.split("\\n").join("\\n> ")}\\n\\n` });
            setSelectionBox(null);
          }}
        >
          Add Comment
