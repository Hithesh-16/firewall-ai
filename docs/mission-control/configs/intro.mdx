...
<Info>
  **New UI Update: The Hub is now Mission Control.**
  If you're returning after the redesign, you can now find and manage configs in your [Mission Control Settings](https://ai-firewall.dev/settings/configs).
</Info>

## What Are Configs?

Configs are flexible containers that combine multiple components to create powerful AI coding experiences using the AI Firewall CLI or the Extensions. A single config can include:
...
<Steps>
  ...
  <Step title="Explore existing configs">
    ...
    Browse the [AI Firewall Mission Control](https://ai-firewall.dev/hub?type=configs) to see what configs others have built
  ...
  <Step title="Use in your IDE">
    ...
    Select the config from the dropdown in your AI Firewall extension, type `/config` in the CLI, or use `--config` with the CLI commands
