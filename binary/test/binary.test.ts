...
const AI_FIREWALL_GLOBAL_DIR = path.join(__dirname, "..", ".ai-firewall");
if (fs.existsSync(AI_FIREWALL_GLOBAL_DIR)) {
  fs.rmSync(AI_FIREWALL_GLOBAL_DIR, { recursive: true, force: true });
}
fs.mkdirSync(AI_FIREWALL_GLOBAL_DIR);
...
describe("Test Suite", () => {
    ...
    if (USE_TCP) {
      ...
      try {
        subprocess = spawn(binaryPath, {
          env: { ...process.env, AI_FIREWALL_GLOBAL_DIR },
        });
  ...
  it("should create .continue directory at the specified location with expected files", async () => {
    expect(fs.existsSync(AI_FIREWALL_GLOBAL_DIR)).toBe(true);
    ...
    const missingFiles = expectedFiles.filter((file) => {
      const filePath = path.join(AI_FIREWALL_GLOBAL_DIR, file);
