...
sidebarTitle: "PostHog Analytics with AI Firewall CLI"
---
...
<Card title="What You'll Build" icon="robot">
  A fully automated workflow that uses AI Firewall CLI with the PostHog MCP to fetch analytics data, analyze user experience issues with AI, and automatically create GitHub
...
<Steps>
  ...
  <Step title="Add Required Secrets to AI Firewall CLI">
    AI Firewall CLI will securely store your API keys as secrets that can be referenced in prompts.
  </Step>
</Steps>

<Tip>
  AI Firewall CLI handles the complex API interactions - you just need to provide
  the right prompts!
</Tip>

## Step 1: Set Up Your Credentials

First, you'll need to gather your PostHog and GitHub API credentials and add them as secrets in AI Firewall CLI.
...
<Tabs>
  <Tab title="PostHog API Credentials">
    ...
    3. Name it "AI Firewall CLI Session Analysis"
    ...
    8. You'll also need your POSTHOG_AUTH_HEADER value, which is simply `Bearer YOUR_API_KEY`
      ...
      <Info>
  **AI Firewall Secrets**: The `POSTHOG_AUTH_HEADER` secret should be stored in
  AI Firewall's secure secrets storage. This keeps your API key safe and the MCP
...
<Tab title= "Configure API Access in AI Firewall CLI">
 
<Tip>
See the [secret types documentation](/mission-control/secrets/secret-types) for adding secrets
</Tip>

You only need to configure the PostHog MCP credential - it automatically handles project selection. To add environment variables to your AI Firewall Mission Control account:

1.  Go to the [AI Firewall Mission Control](https://ai-firewall.dev)
...
<Tabs>
    ...
    **Perfect for:** Immediate results with optimized prompts and built-in debugging
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        Visit the [PostHog Continuous AI Agent](https://ai-firewall.dev/ai-firewall/posthog-continuous-ai-agent) on AI Firewall Mission Control and click **"Install Agent"** or run:
        
        ```bash
        cn --agent ai-firewall/posthog-continuous-ai-agent
        ```
  ...
  <Steps>
    <Step title="Add PostHog MCP to AI Firewall CLI">
      First, install the [PostHog MCP](https://ai-firewall.dev/posthog/http-mcp) to the AI Firewall CLI by
    ...
    <Step title="Add PostHog + GitHub Analysis Rules">
      Visit [PostHog GitHub Continuous AI Rules](https://ai-firewall.dev/bekah-hawrot-weigel/posthog-github-continuous-ai-rules) and click "Use Rule" to add it to your Config.
      
      You could also
      1. Pass `--rule bekah-hawrot-weigel/posthog-github-continuous-ai-rules` to `cn` OR
      2. Copy the rules to your local `.ai-firewall/rules` folder. See the [Rules Guide](/customize/deep-dives/rules#how-to-create-rules).
    </Step>
    <Step title="Create Your Custom Prompts">
      Use this prompt with AI Firewall CLI to analyze PostHog data and create GitHub issues:
...
Here are practical examples of what you can build with PostHog MCP and AI Firewall CLI:
...
**Example AI Firewall CLI prompts:**
...
Consider enhancing your workflow with these advanced AI Firewall CLI prompts:
...
- Consider [GitHub MCP](https://ai-firewall.dev/github/github-mcp) as an alternative (note: can be token-expensive)
- Configure [Slack MCP](https://ai-firewall.dev/slack/slack-mcp) for alerts
...
- [GitHub MCP on AI Firewall Mission Control](https://ai-firewall.dev/github/github-mcp) (alternative option)
