...
Persistent permissions are stored in `~/.ai-firewall/permissions.yaml`. This file is updated when you choose "AI Firewall + don't ask again" in the TUI approval prompt.

```yaml
# ~/.ai-firewall/permissions.yaml
allow:
  - Read(*)
  - Write(**/*.ts)
