...
export async function formatMessageWithFiles(
  ...
  // Filter out restricted files before including in context
  const allowedFiles = attachedFiles.filter((file) => {
    try {
      const { isSecurityConcern } = require("core/indexing/ignore.js");
      if (isSecurityConcern(file.path)) {
        logger.warn(`CLI: Skipping restricted file: ${file.path}`);
        return false;
      }
    } catch {
      // Core module not available — proceed without local check (proxy enforces)
    }
    return true;
  });

  // Convert attached files to context items
  const contextItems = allowedFiles.map((file) => ({
    id: {
      providerTitle: "file",
      itemId: uuidv4(),
    },
