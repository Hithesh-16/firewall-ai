---
title: "Building Data Pipelines with dlt MCP and AI Firewall"
...
sidebarTitle: "dlt Data Pipelines with AI Firewall"
---
...
<Card title="What You'll Build" icon="database">
  An AI-powered data pipeline development system that uses AI Firewall's AI agent with dlt
...
<Card title="🚀 Fastest Path to Success" icon="zap">
  Skip the manual setup and use our pre-built [dlt Agent](https://ai-firewall.dev/ai-firewall/dlt-agent) that includes
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        ```bash
        cn --agent ai-firewall/dlt-agent
        ```
...
<Steps>
  <Step title="Create a New Agent via the AI Firewall Mission Control">
    Go to the [AI Firewall Mission Control](https://ai-firewall.dev) and [create a new agent](https://ai-firewall.dev/agents/new).
    </Step>

  <Step title="Connect dlt MCP via AI Firewall Mission Control">
    Visit the [dlt MCP on AI Firewall Mission Control](https://ai-firewall.dev/dlthub/dlt-mcp) and click **Install** to add it to the agent you created in the step above.
    ...
    <Tip>
      **Alternative installation methods:**
      ...
      2. **Manual configuration**: Add the MCP to your `~/.ai-firewall/config.json` under the `mcpServers` section

      Once installed, dlt MCP tools become available to your AI Firewall agent for all prompts.
    </Tip>
...
<Accordion title="Agent Requirements">
    To use the pre-built [dlt Agent](https://ai-firewall.dev/ai-firewall/dlt-agent), you need either:
      - **AI Firewall CLI Pro Plan** with the models add-on, OR
      - **Your own API keys** added to AI Firewall Mission Control secrets (same as manual setup)
...
<Card title="Understanding the Difference" icon="info-circle">
  ...
  **[dlt+ MCP](https://ai-firewall.dev/dlthub/dlt-plus-mcp)** extends these capabilities with cloud-based features for production deployments:
  ...
  For local development and getting started, **[dlt MCP](https://ai-firewall.dev/dlthub/dlt-mcp)** is the right choice. Consider **[dlt+ MCP](https://ai-firewall.dev/dlthub/dlt-plus-mcp)** when you need production deployment features and team collaboration.
</Card>
...
Now you can use natural language prompts to develop and debug your dlt pipelines. The AI Firewall agent automatically calls the appropriate dlt MCP tools.

<Info>
You can add prompts to your agent's configuration for easy access in future sessions. Go to your agent in the [AI Firewall Mission Control](https://ai-firewall.dev), click **Edit**, and add prompts under the **Prompts** section.
</Info>

<Info>
  **Where to run these workflows:**
  - **IDE Extensions**: Use AI Firewall in VS Code, JetBrains, or other supported IDEs
...
## Continuous Data Pipelines with GitHub Actions
  ...
  This example demonstrates a **Continuous AI workflow** where data pipeline validation runs automatically in your CI/CD pipeline in headless mode using the [dlt Assistant agent](https://ai-firewall.dev/dlthub/dlt-assistant). Consider [remixing this agent](/guides/understanding-configs#how-to-get-started-with-hub-configs) to add your organization's specific validation rules.

### Add GitHub Secrets

Navigate to **Repository Settings → Secrets and variables → Actions** and add:

- `CONTINUE_API_KEY`: Your AI Firewall API key from [ai-firewall.dev/settings/api-keys](https://ai-firewall.dev/settings/api-keys)
- Any required database credentials for your destination

<Info>
      The workflow uses the pre-built [dlt Agent](https://ai-firewall.dev/ai-firewall/dlt-agent) with `--agent ai-firewall/dlt-agent`. This agent comes pre-configured with the dlt MCP and optimized rules for pipeline operations. You can [remix this agent](/guides/understanding-configs#how-to-get-started-with-hub-configs) to customize the validation rules and prompts for your specific pipeline requirements.
</Info>

### Create Workflow File

This workflow automatically validates your dlt data pipelines on pull requests using the AI Firewall CLI in [headless mode](/cli/headless-mode). It inspects pipeline schemas, checks for errors, and posts a summary report as a PR comment. The workflow can also be triggered manually via `workflow_dispatch`.
    ...
    steps:
      ...
      - name: Install AI Firewall CLI
        run: |
          ...
          echo "✅ AI Firewall CLI installed"
      ...
      - name: Comment Pipeline Report on PR
        ...
        run: |
          REPORT=$(cn --agent ai-firewall/dlt-agent \
...
Implement automated pipeline quality checks using AI Firewall's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
...
<Check>
  **Verification Steps:**
  - dlt MCP is installed via [AI Firewall Mission Control](https://ai-firewall.dev/dlthub/dlt-mcp)
...
<CardGroup cols={2}>
  ...
  <Card title="AI Firewall Mission Control" icon="plug" href="https://ai-firewall.dev">
  ...
  >
    Learn about AI agents, MCP, and AI Firewall integration
  </Card>
  <Card
    title="MCP Concepts"
    icon="puzzle-piece"
