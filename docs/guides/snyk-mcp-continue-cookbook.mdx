---
title: "Automated Security Scanning with Snyk MCP and AI Firewall"
...
sidebarTitle: "Snyk Security Scanning with AI Firewall"
---
...
<Card title="What You'll Build" icon="shield-halved">
  An automated security scanning system that uses AI Firewall's AI agent with Snyk
...
<iframe
  ...
  title="Snyk MCP AI Firewall Cookbook Demo"
  frameborder="0"
...
<Note>
  **Important**: The Snyk MCP requires the Snyk CLI to be authenticated locally. Run `snyk auth` to authenticate before using the AI Firewall agent with Snyk MCP.
</Note>
 <Warning>
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        ```bash
        cn --agent ai-firewall/snyk-continuous-ai-agent
        ```
...
<Steps>
  ...
  <Step title="Connect Snyk MCP via AI Firewall Mission Control">
    Visit the [Snyk Continuous AI Agent](https://ai-firewall.dev/ai-firewall/snyk-continuous-ai-agent) and click **Install** to add it to the agent you created in the step above.
    ...
    <Tip>
      **Alternative installation methods:**
      ...
      Once installed, Snyk MCP tools become available to your AI Firewall agent for all prompts.
    </Tip>

    <Info>
      The MCP will request authentication and folder trust permissions when first used.
      This is handled automatically by the AI Firewall agent.
    </Info>

  </Step>

  <Step title="Add Secure-at-Inception Rules">
    Install the [Snyk Secure-at-Inception rules](https://ai-firewall.dev/snyk/secure-at-inception) from Mission Control to enable automatic security scanning.
    ...
    **How to add rules to your agent:**
    ...
    3. Rules apply globally to all your AI Firewall sessions
...
Now you can use natural language prompts to run comprehensive security scans. The AI Firewall agent automatically calls the appropriate Snyk MCP tools.
    ...
    steps:
      ...
      - name: Generate AI Mitigation Suggestions
        ...
        run: |
          ...
          # Create a summary of findings for AI Firewall CLI
          ...
          if [ -n "$FINDINGS_SUMMARY" ]; then
            ...
            # Use AI Firewall CLI to generate mitigation suggestions
            ...
            cn --agent ai-firewall/snyk-continuous-ai-agent -p "$PROMPT" --auto > mitigation-suggestions.md || {
      ...
      - name: Post Mitigation Summary to PR
        ...
        run: |
          ...
          **Scan Details:**
          ...
          - 🤖 Generated with AI Firewall CLI + Snyk
...
<Info>
  **About SNYK_TOKEN**: The workflow uses the SNYK_TOKEN in two ways:
  ...
  2. **AI Firewall CLI access** - Available as an environment variable when AI Firewall generates AI mitigation suggestions
...
<Info>
  ...
  - **AI Mitigation**: Uses AI Firewall CLI to generate actionable mitigation steps
...
Implement automated security policies using AI Firewall's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.

<Note>
  **Coming Soon**: These security guardrail prompts will be available as pre-configured rules on the AI Firewall Mission Control for easy installation.
</Note>
...
<Check>
  **Verification Steps:** - Snyk MCP is installed via [AI Firewall
  Hub](https://ai-firewall.dev/snyk/snyk-mcp) - Secure-at-Inception rules are
  [enabled](https://ai-firewall.dev/snyk/secure-at-inception) - Authentication
