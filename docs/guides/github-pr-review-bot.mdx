---
title: "Code Review Bot with AI Firewall and GitHub Actions"
description: "Set up automated, context-aware pull request reviews using AI Firewall CLI in GitHub Actions - privacy-first with custom rules"
...
<CardGroup cols={3}>
  <Card title="Privacy-First" icon="shield">
    All logs and processing happen in your runner: AI Firewall CLI runs in GitHub Actions → code to your LLM provider (OpenAI, Anthropic, etc.). No hosted AI Firewall service reads your code.

  </Card>

  <Card title="Customizable" icon="sliders">
    Define team-specific rules in `.ai-firewall/rules/` that automatically apply to every pull request.

  </Card>

  <Card title="Context Awareness" icon="robot">
    Leverage AI Firewall's AI agent for intelligent, context-aware reviews with full control over your configuration.
...
- A AI Firewall API key from [ai-firewall.dev/settings/api-keys](https://ai-firewall.dev/settings/api-keys)
- AI Firewall assistant configured for code reviews (or use our recommended default)

<Info>
  **Want to customize the review bot?**
  
  You can remix the default review bot configuration at [ai-firewall.dev/ai-firewall/review-bot](https://ai-firewall.dev/ai-firewall/review-bot) to create your own personalized version with custom prompts, rules, and behaviors.
</Info>
...
jobs:
  review:
    ...
    steps:
      ...
      - name: Run AI Firewall Review
        env:
          CONTINUE_API_KEY: ${{ secrets.CONTINUE_API_KEY }}
          GH_TOKEN: ${{ steps.app-token.outputs.token || github.token }}
        run: |
          # Check if custom rules exist
          if [ -d ".ai-firewall/rules" ]; then
            echo "📋 Found custom rules in .ai-firewall/rules/"
            RULES_CONTEXT="Apply the custom rules found in .ai-firewall/rules/ directory."
          else
          ...
          # Run AI Firewall CLI in headless mode
          cn --config ai-firewall/review-bot \
             -p "$PROMPT" \
      ...
      - name: Post Review Comment
        ...
        run: |
          ...
          ---
          *Powered by [AI Firewall](https://ai-firewall.dev) • Need a focused review? Comment `@review-bot check for [specific concern]`*
          EOF
...
Define your team's standards in `.ai-firewall/rules/`:

<Tabs>
  <Tab title="Security Rule">
    
 Create `.ai-firewall/rules/security.md`:
...
```
  ...
  <Tab title="TypeScript Standards">
    Create `.ai-firewall/rules/typescript.md`:
...
Create `.ai-firewall/rules/testing.md`:
...
Create `.ai-firewall/rules/python.md`:
...
3. **Load Custom Rules** - Reads your team's rules from `.ai-firewall/rules/`
4. **Get Pull Request Diff** - Fetches the diff and list of changed files
5. **AI Firewall CLI Analyzes Code** - AI agent reviews the code with your rules
...
<Tabs>
  <Tab title="Use Your Own AI Firewall Config">
    
  By default, the workflow uses the `ai-firewall/review-bot` config optimized for code reviews. Replace `ai-firewall/review-bot` with your own config:
...
## Troubleshooting
<AccordionGroup>
<Accordion title="AI Firewall CLI not found">
- The workflow installs the CLI automatically, but ensure Node.js 20+ is available
- Check the "Install AI Firewall CLI" step logs for errors
</Accordion>
...
- Verify your AI Firewall config is accessible
- Check AI Firewall CLI logs in the workflow run
...
>
> ### Recommendations
> 1. Move `secretKey` to environment variables (see `.ai-firewall/rules/security.md`)
...
>
> ---
> *Powered by [AI Firewall](https://ai-firewall.dev) • Need a focused review? Comment `@review-bot check for security`*
...
5. **Create team config** - Set up a shared AI Firewall config for consistent reviews
...
<CardGroup cols={2}>
  ...
  <Card title="AI Firewall CLI Guide" icon="terminal" href="/guides/cli">
    Learn more about AI Firewall CLI capabilities
  </Card>
...
- Tweet your setup with `#AIFirewall`
- Share improvements as [GitHub discussions](https://github.com/ai-firewall/ai-firewall/discussions)
