import { FQSN, SecretResult, SecretType } from "@ai-firewall/config-yaml";
import {
  afterEach,
  beforeEach,
  describe,
...
describe("LocalPlatformClient", () => {
  ...
  describe("searches for secrets in local .env files", () => {
    let getAiFirewallDotEnv: Mock;
    beforeEach(async () => {
      const utilPaths = await import("../../util/paths");
      getAiFirewallDotEnv = vi.fn(() => envKeyValues);
      utilPaths.getAiFirewallDotEnv = getAiFirewallDotEnv;
    });

    test("should be able to get secrets from ~/.ai-firewall/.env files", async () => {
      const localPlatformClient = new LocalPlatformClient(
        null,
        testControlPlaneClient,
        testIde,
      ...
      const resolvedFQSNs = await localPlatformClient.resolveFQSNs([testFQSN]);
      expect(getAiFirewallDotEnv).toHaveBeenCalled();
  ...
  describe("should be able to get secrets from workspace .env files", () => {
    test("should get secrets from <workspace>/.ai-firewall/.env and <workspace>/.env", async () => {
    ...
    test("should first get secrets from <workspace>/.ai-firewall/.env and then <workspace>/.env", async () => {
      const originalIdeFileExists = testIde.fileExists;
      testIde.fileExists = vi.fn(async (fileUri: string) =>
        fileUri.includes(".env") ? true : originalIdeFileExists(fileUri),
      );
  ...
  describe("should be able to get secrets from process.env", () => {
    ...
    beforeEach(async () => {
      ...
      const utilPaths = await import("../../util/paths");
      utilPaths.getAiFirewallDotEnv = vi.fn(() => ({}));
    ...
    test("should prioritize local ~/.ai-firewall/.env file over process.env", async () => {
