...
`,
  ...
  test("should not fall back to default config when explicit config fails", async () => {
    ...
    try {
      ...
      expect(message).not.toContain("~/.ai-firewall/config.yaml");
...
describe("AI_FIREWALL_USE_BEDROCK environment variable", () => {
  const mockConsoleLog = vi.fn();
  let mockAuthConfig: AuthConfig;
  const originalEnv = process.env.AI_FIREWALL_USE_BEDROCK;
  ...
  afterEach(() => {
    if (originalEnv) {
      process.env.AI_FIREWALL_USE_BEDROCK = originalEnv;
    } else {
      delete process.env.AI_FIREWALL_USE_BEDROCK;
    }
    vi.restoreAllMocks();
    vi.doUnmock("./config.js");
  });

  test("should bypass interactive options when AI_FIREWALL_USE_BEDROCK=1", async () => {
    process.env.AI_FIREWALL_USE_BEDROCK = "1";
    ...
    expect(mockConsoleLog).toHaveBeenCalledWith(
      expect.stringContaining(
        "✓ Using AWS Bedrock (AI_FIREWALL_USE_BEDROCK detected)",
      ),
    );
  });

  test("should not bypass when AI_FIREWALL_USE_BEDROCK is not '1'", async () => {
    process.env.AI_FIREWALL_USE_BEDROCK = "0";
