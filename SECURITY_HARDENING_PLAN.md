...
- ✅✓ **C1 (`GET /api/providers/by-slug/:slug`)** — added to `proxy/src/routes/provider.route.ts`. Composes the existing `resolveProviderForUser(userId, orgId, slug)` + `getProviderBySlug` services and returns `{slug, baseUrl, source: "user"|"org"|"global", decryptedKey}` to authenticated callers. Resolution order matches `gatewayRouter`: user override → org default → global registry. Returns 404 with a clear "no vault entry resolves this slug" message if nothing matches — failing closed per the C-phase principle. Gated by the `provider:read` capability (same as the existing `GET /api/providers`).
- ✅✓ **C5 (GUI wizard vaults the key first)** — `gui/src/components/OnboardingCard/hooks/useSubmitOnboarding.ts` now POSTs the apiKey to `localhost:8080/api/providers` BEFORE telling the IDE about it. The plaintext key never leaves the GUI process — the IDE messenger receives only the reference shape `vault://<slug>`. Proxy unreachable → user sees a clear alert + onboarding aborts (no silent downgrade). This brings the IDE webview into parity with the standalone `web/` dashboard's `Step5Providers.tsx` which already uses `apiClient.post(ENDPOINTS.providers.root, …)`.
- ✅✓ **C2 (Onboarding writes `apiKeyRef`, not raw `apiKey`)** — `core/config/onboarding.ts:setupProviderConfig(config, provider, apiKeyRef)` rewritten. Drops the `uses + with: { OPENAI_API_KEY: apiKey }` shape (which leaked plaintext into YAML inputs) in favour of flat `{ provider, model, apiKeyRef }` model entries. Updated `core/core.ts:handleCompleteOnboarding` to forward `apiKeyRef` instead of `apiKey`, and `CompleteOnboardingPayload` (`core/index.d.ts:1951`) now declares `apiKeyRef?: string` only — the legacy `apiKey` field is gone from the type so any straggling caller fails type-check.
- ✅✓ **C3 (BaseLLM resolver)** — `core/llm/index.ts` keystone change. Added `apiKeyRef?: string` to the `LLMOptions` interface (`core/index.d.ts:LLMOptions`) and a lazy `ensureApiKeyResolved()` method on `BaseLLM` that fetches `GET /api/providers/by-slug/:slug` (Phase C.C1), caches the in-flight Promise to coalesce concurrent requests, mutates `this.apiKey` with the resolved value, and rebuilds `this.openaiAdapter`. Constructor accepts BOTH `apiKeyRef` (preferred) and `apiKey` (legacy raw); when `apiKey` itself is a `vault://` reference, it's promoted to `apiKeyRef` and the placeholder is cleared so it never leaks as a Bearer token. `streamChat` awaits `ensureApiKeyResolved()` at the top so the existing 60+ provider subclasses keep reading `this.apiKey` synchronously without changes — V3 (every provider uses the field in headers) closes for free as the field gets the resolved value before each request.
- ✅✓ **C6 (HARD REFUSAL)** — `core/config/yaml/loadYaml.ts` now adds a fatal `ConfigValidationError` for any model whose `apiKey:` value is a literal string (not `vault://...`, not `${{ secrets.X }}`, not `${ENV_VAR}` / `$ENV_VAR`). The classifier lives in a new dependency-free module `core/config/yaml/apiKeyClassifier.ts` so it can be unit-tested without the loader's transitive deps. 22 new Jest tests in `loadYaml.c6.test.ts` cover every documented acceptable shape plus the obvious leak shapes (sk-proj/sk-ant/gsk\_/AIza/random literals/whitespace-padded). The error message names the offending model(s) and points at the migration path. Phase A.A2's `ScanReport` still fires too (informational) for users who want a banner before the load actually fails.

**Phase C — verdict (2026-04-17):** C1 + C2 + C3 + C5 + C6 + C7 are all done. C4 was already closed (V4/V5 noted in the survey). All four UI surfaces — VS Code webview, web standalone settings, web standalone onboarding, CLI — converge on the same `POST /api/providers` write path; raw `apiKey:` in YAML now hard-blocks the load; BaseLLM resolves vault references at the proxy boundary. The vault is now authoritative.

`tsc --noEmit` clean across `core/`, `proxy/`, `gui/`, `extensions/cli/`. Proxy test baseline: 718 pass / 1 known pre-existing fail. Core test addition: 22 new C6 tests, all green.
