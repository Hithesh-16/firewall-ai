...
Check files live in `.ai-firewall/checks/` or `.agents/checks/` at the repository root.

## Frontmatter fields

| Field | Required | Type | Description |
|-------|----------|------|-------------|
| `name` | Yes | string | Display name shown in GitHub status checks and on ai-firewall.dev |
...
```markdown .ai-firewall/checks/migration-safety.md
---
name: Migration Safety
description: Flag destructive database migrations
...
AI Firewall locates checks by scanning specific directories at the repository root:

- Reads all `.md` files in `.ai-firewall/checks/`
...
`.ai-firewall/checks/` and `.agents/checks/` are equivalent. Both are scanned and merged into a single list of checks.

`.agents/` is an emerging cross-tool standard for AI agent configuration. `.ai-firewall/` is AI Firewall-specific. Use whichever fits your project. If both directories contain a check with the same `name`, the `.ai-firewall/checks/` version takes precedence.
