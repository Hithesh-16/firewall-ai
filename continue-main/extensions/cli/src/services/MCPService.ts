import { decodeFQSN, getTemplateVariables } from "@ai-firewall/config-yaml";
...
For personal use you can set the secret in the hub at https://ai-firewall.dev/settings/secrets or pass it to the CLI environment.
Org-level secrets can only be used for MCP by Background Agents (https://docs.ai-firewall.dev/hub/agents/overview) when \"Include in Env\" is enabled for the secret.`;
        if (this.isHeadless) {
