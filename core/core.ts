...
export class Core {
  ...
  private registerMessageHandlers(ideSettingsPromise: Promise<IdeSettings>) {
    ...
    on("history/repoSummary", (msg) => {
      ...
      return {
        summary,
        formatted: formatRepoSummaryForSystemMessage(summary),
      };
    });

    on("history/clearRepoSummary", (msg) => {
      const { clearRepoSummary } = require("./util/repoMemory");
      return clearRepoSummary(msg.data.workspaceDirectory);
    });
    ...
    if (data?.uris?.length) {
      ...
      for (const uri of data.uris) {
        ...
        if (isColocatedRulesFile(uri)) {
          ...
          uri.endsWith(".ai-firewallignore") ||
  ...
  private getContextItems = async (
    ...
    try {
      ...
      if (e instanceof Error) {
        ...
        //           "https://docs.ai-firewall.dev/customize/model-roles/embeddings",
        //         );
        //       }
        //     });
        // }
