---
title: "Content Management with Sanity MCP and AI Firewall"
...
sidebarTitle: "Sanity CMS with AI Firewall"
---
...
<Card title="What You'll Build" icon="database">
  An AI-powered content management system workflow that uses AI Firewall's AI agent with Sanity
...
<Step title="Install Sanity CLI">
 ...
 <Warning>
  To use agents in headless mode, you need a [AI Firewall API key](https://ai-firewall.dev/settings/api-keys) and proper environment variable configuration.
  </Warning>
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        Visit the [Sanity Assistant Agent](https://ai-firewall.dev/ai-firewall/sanity-assistant-agent) on AI Firewall Mission Control and click **"Install Config"** or run:

        ```bash
        cn --agent ai-firewall/sanity-assistant-agent
        ```

        This agent includes:
        - **[Sanity MCP](https://ai-firewall.dev/sanity/sanity-mcp)** pre-configured and ready to use
...
<Steps>
  ...
  <Step title="Connect Sanity MCP via AI Firewall Mission Control">
    Visit the [Sanity MCP on AI Firewall Mission Control](https://ai-firewall.dev/sanity/sanity-mcp) and click **"Install"** to add it to the agent you created in the step above.
    ...
    <Tip>
      **Alternative installation methods:**
      ...
      2. **With config**: Use [Sanity MCP Config](https://ai-firewall.dev/sanity/sanity-mcp-config) for environment variable setup
      3. **Manual configuration**: Add the MCP to your agent configuration

      Once installed, Sanity MCP tools become available to your AI Firewall agent for all prompts.
    </Tip>
    ...
    <Info>
      **Authentication Options:**
      ...
      See [Sanity MCP Config](https://ai-firewall.dev/sanity/sanity-mcp-config) for environment variable setup.
    </Info>
...
Now you can use natural language prompts to manage your Sanity content and schemas. The AI Firewall agent automatically calls the appropriate Sanity MCP tools.
...
<Info>
  The workflow uses the [Sanity Assistant Agent](https://ai-firewall.dev/ai-firewall/sanity-assistant-agent) with environment variable authentication via [Sanity MCP Config](https://ai-firewall.dev/sanity/sanity-mcp-config). This enables headless mode operation without OAuth browser authentication.
</Info>

### Create Workflow File

This workflow automatically validates your Sanity schemas and content on pull requests using the AI Firewall CLI in [headless mode](/cli/headless-mode). It checks schema integrity, validates content relationships, and posts a summary report as a PR comment.
  ...
  validate-content:
    ...
    steps:
      ...
      - name: Comment Report on PR
        ...
        run: |
          REPORT=$(          cn --agent ai-firewall/sanity-assistant-agent \
...
<Info>
  Environment variables enable the MCP to authenticate without OAuth browser prompts. The [Sanity MCP Config](https://ai-firewall.dev/sanity/sanity-mcp-config) documentation provides detailed setup instructions for all required variables.
</Info>

## Content Management Best Practices

Implement automated content quality checks using AI Firewall's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
...
<Check>
  **Verification Steps:**
  - Sanity MCP is installed via [AI Firewall Mission Control](https://ai-firewall.dev/sanity/sanity-mcp)
