...
function fallbackRender({ error, resetErrorBoundary }: any) {
  ...
  return (
    ...
    >
      ...
      <pre className="text-description">{error.stack}</pre>
...
export function Chat() {
  ...
  const canRevert =
    useAppSelector((s) => s.session.historySnapshot !== null) && !isStreaming;
  ...
  const handleStickyEdit = useCallback((index: number) => {
    ...
    const focusEditor = () => {
      const editorEl = promptEl.querySelector(
        ".tiptap.ProseMirror",
      ) as HTMLElement;
      editorEl?.focus();
    };
    container.addEventListener("scrollend", focusEditor, { once: true });
  }, []);
  ...
  const sendInput = useCallback(
    (
      ...
      if (isCurrentlyInEdit) {
        ...
        // New message (not retry/edit) — user accepted current state, clear snapshot
        if (!index && stateSnapshot.session.historySnapshot) {
          dispatch(clearHistorySnapshot());
        }
  ...
  const handleStickyRetry = useCallback(
    (index: number) => {
    ...
    [dispatch, history, sendInput],
  );

  const handleStickyRevert = useCallback(() => {
    dispatch(revertToSnapshot());
  }, [dispatch]);
