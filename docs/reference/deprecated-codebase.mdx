...
Continue also supports a **global** `.ai-firewallignore` file that will be respected for all workspaces, which can be created at `~/.ai-firewall/.ai-firewallignore`.

If you want to see exactly what files Continue has indexed, the metadata is stored in `~/.ai-firewall/index/index.sqlite`. You can use a tool like [DB Browser for SQLite](https://sqlitebrowser.org/) to view the `tag_catalog` table within this file.
