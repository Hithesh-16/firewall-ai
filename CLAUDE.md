...
- **Inline File Scan Reports** — File-read tools (`readFile`, `readFileRange`, `readCurrentlyOpenFile`, CLI counterparts) call the proxy's `/api/scan/file`, then append a second `ContextItem` named "AI Firewall" carrying a markdown report with file:line:column for every secret/PII finding. Replaces the old VS Code toast that showed type counts but no file context. Powered by `proxy/src/scanner/findingLocator.ts` (line/col binary search + type-aware masking) and `core/util/formatScanFindings.ts` (markdown formatter). For BLOCK results the same report is attached to the thrown `ContinueError` so the error renderer shows file/line context too.
