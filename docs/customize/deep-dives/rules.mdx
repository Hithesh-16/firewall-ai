---
title: "How to Create and Manage Rules in AI Firewall"
...
AI Firewall supports two types of rules with different behaviors:

<Columns cols={2}>
<Card title="Local Rules" icon="folder">
- **Location**: `.ai-firewall/rules` folder in your workspace
...
- **Location**: Stored on AI Firewall Mission Control, referenced in config.yaml
...
When using AI Firewall, rules are loaded in this order:
1. **Hub assistant rules** (if using a Hub-based assistant)
2. **Referenced Hub rules** (via `uses:` in config.yaml)
3. **Local workspace rules** (from `.ai-firewall/rules` folder)
4. **Global rules** (from `~/.ai-firewall/rules` folder)
...
1. Create a folder called `.ai-firewall/rules` at the top level of your workspace
2. Add a file called `pirates-rule.md` to this folder.
3. Write the following contents to `pirates-rule.md` and save.

```md title=".ai-firewall/rules/pirates-rule.md"
---
name: Pirate rule
---
...
For example, you can say "Create a rule for this", and a rule will be created for you in `.ai-firewall/rules` based on your conversation.
</Info>

### Creating Hub Rules

Rules can also be created and managed on the AI Firewall Mission Control:

1. **Browse existing rules**: [Explore available rules](https://ai-firewall.dev/hub?type=rules)
2. **Create new rules**: [Create your own](https://ai-firewall.dev/new?type=block&blockType=rules) in the Hub
3. **Copy from local rules**: Copy/paste content from your `.ai-firewall/rules` files to create Hub rules
...
1. Copy the content from your `.ai-firewall/rules/rule-name.md` file
2. Go to [Create new rule](https://ai-firewall.dev/new?type=block&blockType=rules)
...
globs: docs/**/*.{md,mdx}
alwaysApply: false
description: Standards for writing and maintaining AI Firewall Docs
---

# AI Firewall Docs Standards
...
You can create project-specific rules by adding a `.ai-firewall/rules` folder to the root of your project and adding new rule files.
...
```md title=".ai-firewall/rules/typescript.md"
---
...
**Root Cause**: This happens when you have a mix of local and Hub rules, and AI Firewall can't properly determine where each rule originates.

**Workaround**:
1. **For local rules**: Navigate directly to `.ai-firewall/rules/` folder and edit the `.md` file
2. **For Hub rules**: Go directly to your assistant page on [AI Firewall Mission Control](https://ai-firewall.dev) and edit from there
...
**Check These**:
1. **File location**: Ensure local rules are in `.ai-firewall/rules/` (not `.ai-firewall/rule/`)
...
AI Firewall includes a simple default system message for [Agent mode](../../ide-extensions/agent/quick-start) and [Chat](../../ide-extensions/chat/quick-start) requests, to help the model provide reliable codeblock formats in its output.

This can be viewed in the rules section of the toolbar (see above), or in the source code [here](https://github.com/ai-firewall/ai-firewall/blob/main/core/llm/constructMessages.ts#L4).
