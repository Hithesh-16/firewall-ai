...
In AI Firewall, you can add `edit` to a model's roles to specify that it can be used for Edit requests. If no edit models are specified, the selected `chat` model is used.
...
Explore edit models in [the hub](https://ai-firewall.dev/explore/models?roles=edit). Generally, our recommendations for Edit overlap with recommendations for Chat.
...
You can customize the prompt template used for editing code by setting the `promptTemplates.edit` property in your model configuration. AI Firewall uses [Handlebars syntax](https://handlebarsjs.com/guide/) for templating.
