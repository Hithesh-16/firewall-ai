...
export async function getIgnoreContext(
  ...
  // These are done separately so that .ai-firewallignore can override .gitignore
  const gitIgnoreFile = dirFiles.find((name) => name === ".gitignore");
  const continueIgnoreFile = dirFiles.find(
    (name) => name === ".ai-firewallignore",
  );
  ...
  const getContinueIgnorePatterns = async () => {
    if (continueIgnoreFile) {
      const contents = await ide.readFile(`${currentDir}/.ai-firewallignore`);
      return gitIgArrayFromFile(contents);
    }
    return [];
  };
  ...
  const ignoreContext = ignore()
    ...
    .add(defaultAndGlobalIgnores) // default file/folder ignores followed by global .ai-firewallignore - this is combined for speed
    .add(ignoreArrays[1]); // local .ai-firewallignore
