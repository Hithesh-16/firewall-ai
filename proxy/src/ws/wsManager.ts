...
import { validateApiToken } from "../auth/authService";
 ...
 * Must be called before registerClient. Returns the authenticated userId
 * or null if authentication fails (caller should close the connection).
 */
export function authenticateWsConnection(
  ...
  // Only accept afw_ prefixed tokens (API tokens, not raw provider keys)
  if (!raw.startsWith("afw_")) return null;
 ...
 * Caller MUST authenticate via authenticateWsConnection() first.
 */
