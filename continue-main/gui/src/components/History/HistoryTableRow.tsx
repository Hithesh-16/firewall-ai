...
export function HistoryTableRow({
  ...
  // Pin state — stored in localStorage for cross-platform persistence
  const [isPinned, setIsPinned] = useState(() => {
    try {
      const pinned = JSON.parse(localStorage.getItem("pinnedSessions") || "[]");
      return pinned.includes(sessionMetadata.sessionId);
    } catch { return false; }
  });
  ...
  const togglePin = (e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      const pinned: string[] = JSON.parse(localStorage.getItem("pinnedSessions") || "[]");
      const updated = isPinned
        ? pinned.filter((id: string) => id !== sessionMetadata.sessionId)
        : [...pinned, sessionMetadata.sessionId];
      localStorage.setItem("pinnedSessions", JSON.stringify(updated));
      ...
      <td className="flex-1 cursor-pointer space-y-1">
        {editing ? (
          ...
          <div className="flex items-center gap-2">
            {isPinned && (
              <StarSolid className="h-3 w-3 flex-shrink-0 text-warning" />
            )}
            <span className="line-clamp-1 break-all text-sm font-semibold">
              {sessionMetadata.title}
            </span>
            {isRemote && (
              <span className="inline-flex items-center rounded-full bg-info/10 px-2 py-1 text-xs font-medium text-info dark:bg-info/15 dark:text-info">
                Remote
              </span>
            )}
        ...
        <td className="bg-input absolute right-2 top-12 ml-auto flex -translate-y-1/2 transform items-center gap-x-1 rounded-full px-2 py-1 shadow-md">
          {isRemote ? (
            ...
            <>
              ...
              >
                {isPinned ? (
                  <StarSolid width="1em" height="1em" className="text-warning" />
                ) : (
                  <StarOutline width="1em" height="1em" />
                )}
              </HeaderButtonWithToolTip>
              <HeaderButtonWithToolTip
                text="Edit"
