...
program
  .name("cn")
  .description(
    "AI Firewall CLI - Secure AI-powered development assistant. Starts an interactive session by default, use -p/--print for non-interactive output.",
  )
...
program
  .command("logout")
  .description("Log out from AI Firewall")
  .action(async () => {
    // Telemetry: record command invocation
...
program
  ...
  .option(
    "--id <storageId>",
    "Upload session snapshots to AI Firewall-managed storage using the provided identifier",
  )
  .option(
