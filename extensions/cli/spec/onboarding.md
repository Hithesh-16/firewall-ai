...
3. Present the user with available options:
   ...
   - Enter your Anthropic API key: let them enter the key, and then either create a ~/.ai-firewall/config.yaml with the following contents OR update the existing config.yaml to add the model
...
3. If there are no assistants in that org, then look for a local ~/.ai-firewall/config.yaml
