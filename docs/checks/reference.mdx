...
`.agents/` is an emerging cross-tool standard for AI agent configuration. `.ai-firewall/` is Continue-specific. Use whichever fits your project. If both directories contain a check with the same `name`, the `.ai-firewall/checks/` version takes precedence.
