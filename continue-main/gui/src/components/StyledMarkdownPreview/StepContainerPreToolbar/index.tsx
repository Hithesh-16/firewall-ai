  ...
  const renderActionButtons = () => {
    ...
    if (isGeneratingCodeBlock || isPendingToolCall) {
      ...
      if (isGeneratingCodeBlock) {
        return (
          <span className="text-description-muted inline-flex items-center gap-2 text-right">
            <div>
              <Spinner />
            </div>
          </span>
      ...
      >
        <div className="flex max-w-[50%] flex-row items-center">
          ...
          <ChevronDownIcon
            ...
            className={`text-description-muted h-3.5 w-3.5 flex-shrink-0 cursor-pointer hover:brightness-125 ${
          ...
          {displayFilepath ? (
            ...
            <span className="text-description-muted ml-2 select-none capitalize">
              {language}
            </span>
          )}
        </div>
