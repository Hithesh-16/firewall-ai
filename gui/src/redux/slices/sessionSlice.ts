...
export const sessionSlice = createSlice({
  ...
  reducers: {
    ...
    deleteMessage: (state, action: PayloadAction<number>) => {
      ...
      if (action.payload < 1 || action.payload >= state.history.length) {
        return;
      }
    ...
    togglePlanCollapsed: (state) => {
      if (state.activePlan) {
        state.activePlan.collapsed = !state.activePlan.collapsed;
      }
    },
