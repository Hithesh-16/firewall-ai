...
describe("E2E: Resume Flag", () => {
  ...
  it("should resume from previous session when using --resume flag", async () => {
    ...
    const firstResult = await runCLI(context, {
      ...
      env: {
        ...
        AI_FIREWALL_CLI_TEST_SESSION_ID: "test-session-123",
        AI_FIREWALL_GLOBAL_DIR: path.join(context.testDir, ".ai-firewall"),
      },
      timeout: 15000,
    });
    ...
    const sessionDir = path.join(context.testDir, ".ai-firewall", "sessions");
  ...
  it("should handle --resume when no previous session exists", async () => {
    ...
    const result = await runCLI(context, {
      args: ["-p", "--resume", "--config", context.configPath],
      env: {
        AI_FIREWALL_CLI_TEST_SESSION_ID: "no-session-456",
        AI_FIREWALL_GLOBAL_DIR: path.join(context.testDir, ".ai-firewall"),
      },
      timeout: 15000,
    });
  ...
  it("should resume and allow new messages to be added", async () => {
    ...
    const firstResult = await runCLI(context, {
      ...
      env: {
        AI_FIREWALL_CLI_TEST_SESSION_ID: sessionId,
        AI_FIREWALL_GLOBAL_DIR: path.join(context.testDir, ".ai-firewall"),
      },
      timeout: 15000,
    });
