import { parseConfigYaml } from "@ai-firewall/config-yaml";
import {
    ...
    >
      <div className="flex flex-col">
        <div className="flex flex-row justify-between gap-1">
          <span
            className={`line-clamp-2 ${isDisabled ? "text-description-muted" : "text-vsc-foreground"}`}
            style={{
              fontSize: smallFont,
            }}
          >
        ...
        <span
          ...
          className={`mt-1 line-clamp-3 ${isDisabled ? "text-description" : "text-description-muted"}`}
        >
          {rule.rule}
        </span>
        {rule.globs ? (
          ...
          >
            ...
            <code
              className={`line-clamp-1 px-1 py-0.5 ${isDisabled ? "text-description" : "text-description-muted"}`}
            >
              {rule.globs}
            </code>
          </div>
