import { BlockType, RULE_FILE_EXTENSION } from "@ai-firewall/config-yaml";
...
describe("getFileContent", () => {
  ...
  test("generates correct content for different block types", () => {
    ...
    expect(docsResult).toContain("startUrl: https://docs.ai-firewall.dev");
...
describe("findAvailableFilename", () => {
  test("returns base filename when it doesn't exist", async () => {
    ...
    const result = await findAvailableFilename(
      "/workspace/.ai-firewall/models",
      "models",
      mockFileExists,
    );

    expect(result).toBe("/workspace/.ai-firewall/models/new-model.yaml");
  });

  test("returns filename with counter when base exists", async () => {
    const mockFileExists = async (uri: string) => {
      return uri === "/workspace/.ai-firewall/models/new-model.yaml";
    };
    ...
    expect(result).toBe("/workspace/.ai-firewall/models/new-model-1.yaml");
  });

  test("increments counter until available filename is found", async () => {
    const existingFiles = new Set([
      "/workspace/.ai-firewall/context/new-context.yaml",
      "/workspace/.ai-firewall/context/new-context-1.yaml",
      "/workspace/.ai-firewall/context/new-context-2.yaml",
    ]);
    ...
    const result = await findAvailableFilename(
      "/workspace/.ai-firewall/context",
      "context",
      mockFileExists,
    );

    expect(result).toBe("/workspace/.ai-firewall/context/new-context-3.yaml");
  });
  ...
  test("handles rules markdown files with counter", async () => {
    const existingFiles = new Set([
      `/workspace/.ai-firewall/rules/new-rule.${RULE_FILE_EXTENSION}`,
      `/workspace/.ai-firewall/rules/new-rule-1.${RULE_FILE_EXTENSION}`,
    ]);
    ...
    const result = await findAvailableFilename(
      "/workspace/.ai-firewall/rules",
      "rules",
      mockFileExists,
    );

    expect(result).toBe(
      `/workspace/.ai-firewall/rules/new-rule-2.${RULE_FILE_EXTENSION}`,
    );
  });
  ...
  test("handles large counter values", async () => {
    const existingFiles = new Set(
      Array.from({ length: 100 }, (_, i) =>
        i === 0
          ? "/workspace/.ai-firewall/prompts/new-prompt.md"
          : `/workspace/.ai-firewall/prompts/new-prompt-${i}.md`,
      ),
    );
    ...
    const result = await findAvailableFilename(
      "/workspace/.ai-firewall/prompts",
      "prompts",
      mockFileExists,
    );

    expect(result).toBe("/workspace/.ai-firewall/prompts/new-prompt-100.md");
  });
});
