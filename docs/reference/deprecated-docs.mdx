...
The `@Docs` context provider allows you to efficiently reference documentation directly within AI Firewall.
...
By default, AI Firewall crawls documentation sites using a specialized crawling service that provides the best experience for most users and documentation sites.
...
<Info>
  ...
  to `~/.ai-firewall/.utils`, and use it as the local crawler. **Note:** Chromium
...
- AI Firewall does not automatically re-index your docs. Use `Click to re-index` to trigger a reindex for a specific source
...
You can also use the following IDE command to force a re-index of all docs: `AI Firewall: Docs Force Re-Index`.
